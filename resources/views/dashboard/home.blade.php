    ...
    <div id="comments" class="mt-1">
        @foreach ($todos as $todo)
            <div class="comment d-flex align-items-start justify-content-between">
                ...
                <div class="ml-auto">
                    {{-- ketika akan membuat fitur delete, harus menggunakan form, kenapa? karena kalau kita jalanin fitur delete itu kan artinya mau ubah di datebase nya kan? kalau hal2 yang berhubungan dengan modifikasi database harus menggunakan form--}}
                    <form action="{{ route('todo.delete', $todo['id'])}}" method="POST">
                    @csrf
                    {{-- menimpa attribute method="POST" pada form agar menjadi delete, karena dimethod routernya pake delate --}}
                    @method('DELETE')
                    {{-- biar action formnya bisa dijalani, buttonnya harus type submit --}}
